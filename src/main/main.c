...
Vector2 window2world(Vector2 v)
{
        Vector2 resp = {0};
        resp.x = (v.x - width / 2) / GetScreenWidth();
        resp.y = (v.y - height / 2) / GetScreenHeight();
        return resp;
}
struct inputArgs {
        Camera3D *cam;
...
int gameStuff(__attribute__((unused)) void *args)
{
        ...
        char *vxStr = LoadFileText("resources/main.vs");
        ISNULL(vxStr, ("Error loading vertex shader"));
        char *fgStr = LoadFileText("resources/main.fs");
...
#define forE(index, refvar) for (typeof(refvar) index = 0; index < refvar; index++)
        ...
        while (!WindowShouldClose()) {
                ...
                glBindTexture(GL_TEXTURE_2D, textureRoadId);
                // DrawMesh(floorModel.meshes[0], roadMaterial, MatrixIdentity());

                glUniform1i(glGetUniformLocation(roadShader.id, "ourTexture"), 0);
                glUseProgram(roadShader.id);
                rlSetUniformMatrix(1, mvp);
                // glUniformMatrix4fv(1, 1, GL_FALSE,(GLfloat*)&mvp);
...
{
        // var u0 = (v2){metersFromOriginLat, metersFromOriginLon};
        // var earthRadius = 6371000;
        // var length = Vector2Length(u0);
        // u0 = Vector2Normalize(u0);
        // var poRAD = (v2){pointOfOrigin.x * DEG2RAD, pointOfOrigin.y * DEG2RAD};
        // var phi = atan2(u0.y, u0.x);
        // var resLat=asin(sin(poRAD.x)*cos(length/earthRadius)+cos(poRAD.x)*sin(length/earthRadius)*cos(phi));
        // if (cos(resLat)==0) {
        //         return (v2){0,0};
        // }
        // var preLon=poRAD.y+asin(sin(phi)*sin(length/earthRadius)/cos(resLat))+M_PI;
        // var pre2Lon=fmod(preLon,2*M_PI)-M_PI;
        // return (v2){resLat*RAD2DEG,pre2Lon*RAD2DEG};
